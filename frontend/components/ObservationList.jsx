...
export default function ObservationList({ filters }) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mb-6">
      <h2 className="text-lg font-semibold mb-4 text-primary">Ultimi osservazioni</h2>
      <div className="space-y-3">
        {data.length === 0 && (
          <p className="text-gray-500 text-center py-4">Nessuna osservazione trovata</p>
        )}
        {data.map((obs) => (
          <div key={obs.id} className="flex items-start gap-4 bg-gray-50 p-4 rounded-lg hover:shadow-md transition">
